...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
            ...
            override fun migrate(db: SupportSQLiteDatabase) {
                ...
                db.execSQL(
                    ...
                    CREATE TABLE IF NOT EXISTS arca_config (
                        ...
                        subscriptionPlan TEXT NOT NULL DEFAULT 'Plan Mensual Ilimitado',
                        subscriptionPriceArs REAL NOT NULL DEFAULT 14999.0,
                        subscriptionValidUntil INTEGER NOT NULL DEFAULT 0,
                        subscriptionProvider TEXT NOT NULL DEFAULT 'MERCADO_PAGO'
                    )
                    """.trimIndent()
                )
            }
        ...
        // New file name on purpose: the previous "factuar_v3.db" was plaintext
        // SQLite. SQLCipher can't open a plaintext file with a cipher key, so
        // reusing that name would crash on open for any existing install
        // instead of migrating. A fresh encrypted file sidesteps that; this
        // app predates any real production install, so there's no user data
        // being silently dropped by this rename.
